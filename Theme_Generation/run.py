...
def setup_logging(log_root: str, obj_name: str) -> logging.Logger:
    """Set up logging for the current object processing."""
    os.makedirs(log_root, exist_ok=True)
    log_file = os.path.join(log_root, f"{obj_name}.log")
    ...
    return logging.getLogger(obj_name)


def run_command(command: str, logger: Optional[logging.Logger] = None) -> bool:
    """
    Run a bash command with optional logging.
    
    Args:
        command (str): Command to run
        logger (Optional[logging.Logger]): Logger to use for output
...
def process_image(
    ...
    """
    Process a single image through the 3D generation pipeline.
    
    Args:
        image_name (str): Name of the image to process
        gpu_ids (List[str]): Available GPU IDs
        args (Namespace): Parsed command-line arguments
    
    Returns:
        bool: Whether processing was successful
    """
    # Determine object name and logging
    suffix = "_clipdrop-background-removal" if "_clipdrop-background-removal.png" in image_name else ""
    obj_name = image_name.split(suffix)[0].split(".png")[0]
    logger = setup_logging(args.log_root, obj_name)

    logger.info(f"Processing image: {image_name}")

    # Check if image should be processed based on batch settings
    if args.run_batch:
        image_list = sorted(os.listdir(args.test_img_dir))
        start_ix, end_ix = _get_batch_range(args.batch_range, len(image_list))
        if image not in image_list[start_ix:end_ix]:
            logger.info(f"Skipping {image} - outside batch range")
            return True

    # Prepare pipeline steps
    pipeline_steps = [
        (_get_multi_view, args.get_multi_view),
        (_get_coarse_3d, args.get_coarse_3d),
        (_train_dreambooth, args.train_dreambooth),
        (_get_optim_3d, args.get_optim_3d),
        (_export_mesh, args.export_mesh)
    ]

    # Execute pipeline steps
    for step_func, step_enabled in pipeline_steps:
        if step_enabled:
            if not step_func(obj_name, gpu_ids[0], args, logger):
                logger.error(f"Failed during {step_func.__name__}")
                return False

    logger.info(f"Completed processing for {obj_name}")
    return True
